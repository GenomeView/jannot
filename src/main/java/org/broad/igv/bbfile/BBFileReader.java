...
//			.getLogger(BBFileReader.class.getCanonicalName());
	...
	private boolean isLowToHigh; // BBFile binary data format: low to high or
									// high to low
	private int uncompressBufSize; // buffer byte size for data decompression; 0
									// for uncompressed
	...
	private BPTree chromosomeIDTree; // Container for the mChromosome index B+
										// tree
...
//	}
	...
	public BBFileReader(Locator path, SeekableStream stream)
		...
		fileHeader = new BBFileHeader(fis, fileOffset);
		// fileHeader.print();
		...
		// get number of data records indexed by the R+ chromosome data location
		// tree
...
//	 * 
//	 * Returns: Big Binary File pathname
//	 */
//
...
//	}
	...
	/*
	 * Method returns the Big Binary File header which identifies the file type
	 * and content.
	 * 
	 * Returns: Big Binary File header (Table C)
	 */
	 ...
	 * 
	 * Returns: Boolean identifies if Big Binary File is BigBed (recognized from
	 * magic number in file header Table C)
	 */
	 ...
	 * 
	 * Returns: Boolean identifies if Big Binary File is BigWig (recognized from
	 * magic number in file header Table C)
	 */
	 ...
	 * 
	 * Returns: Count of the total number of compressed/uncompressed data
	 * records: which for BigBed is the number of bed features, and for BiGWifg
	 * is the number of wig sections.
	 */
	 ...
	 * 
	 * Returns: Count of the total number of chromosomes/contigs in the file.
	 */
	 ...
	 * 
	 * Returns: Count of the total number of chromosome/contig regions in the
	 * file.
	 */
	 ...
	 * 
	 * Returns: Largest required buffer size for decompressed data chunks (from
	 * Table C)
	 */
	 ...
	 * 
	 * Returns: Boolean identifies if Big Binary File is low to high byte order
	 * (recognized from magic number in file header Table C); else is high to
	 * low byte order if false.
	 */
	 ...
	 * 
	 * Returns: Total summary block data statistics for the whole file (Table
	 * DD)
	 */
	 ...
	 * 
	 * Returns: B+ Chromosome Index Tree (includes Tables E, F, G, H)
	 */
	 ...
	 * 
	 * Returns: R+ Chromosome Data Locations Tree (includes Tables K, L, M, N)
	 */
	 ...
	 * 
	 * Returns: Number of zoom levels (from Table C)
	 */
	 ...
	 * 
	 * Returns: Zoom level object containing zoom level headers and R+ zoom data
	 * locations tree (includes Tables D, O)
	 */
	 ...
	 * 
	 * Parameters: zoomLevel - zoom level startChromID - start chromosome for
	 * the region endChromID - end chromosome for the region
	 * 
	...
	/*
	 * Method finds chromosome bounds for entire chromosome ID range in the zoom
	 * level R+ tree.
	 * 
	 * Parameters: zoomLevel - zoom level
	 * 
	 * Returns: Chromosome bounds for the entire chromosome ID range in the R+
	 * tree.
	 */
	...
	/*
	 * Method finds chromosome key name for the associated chromosome ID in the
	 * B+ tree.
	 * 
	 * Returns: chromosome key name for associated chromosome ID.
	 */
	 ...
	 * 
	 * Returns: LIst of all chromosome key names in the B+ tree.
	 */
	...
	/*
	 * Returns a chromosome ID which can be used to search for a corresponding
	 * data section in the R+ tree for data.
	 * 
	 * Parameters: chromosomeKey - chromosome name of valid key size.
	 * 
	 * 
	...
	/*
	 * Method finds the chromosome bounding region in R+ tree for a chromosome
	 * ID range.
	 * 
	 * Parameters: startChromID - start chromosome for the region endChromID -
	 * end chromosome for the region
	 * 
	 * Returns: Chromosome region bounds for chromosome ID range
	 */
...
//		assert chromosomeDataTree != null;
	...
	/*
	 * Method finds the chromosome bounds for the entire chromosome ID range in
	 * the R+ tree.
	 * 
	 * Returns: chromosome bounds for the entire chromosome ID range in the R+
	 * tree.
...
//		assert chromosomeDataTree != null;
	...
	private void initChromosomeDataTree() {
	 ...
	 * 
	 * Returns: List of chromosome ID's and regions.
	 */
	 ...
	 * 
	 * Parameters: int zoomLevel - zoom level Returns: List of chromosome ID's
	 * and regions for the zoom level.
	 */
	...
	/**
	 * Returns an iterator for BigBed features which occupy a chromosome
	 * selection region.
	 * <p/>
	 * Note: the BBFile type should be BigBed; else a null iterator is returned.
	 * <p/>
	 * Parameters: startChromosome - name of start chromosome startBase -
	 * starting base position for features endChromosome - name of end
	 * chromosome endBase - ending base position for feature contained - flag
	 * specifies bed features must be contained in the specified base region if
	 * true; else can intersect the region if false
	 * <p/>
	 * Returns: Iterator to provide BedFeature(s) for the requested chromosome
	 ...
	 * Note: the BBFile type should be BigBed; else a null iterator is returned.
	 * <p/>
	 * Returns: Iterator to provide BedFeature(s) for all chromosome regions.
	 * Error conditions: 1) An empty iterator is returned if region has no data
	 * available 2) A null object is returned if the file is not BigBed.(see
	 * isBigBedFile method)
	 */
	...
	/**
	 * Returns an iterator for BigWig values which occupy the specified
	 ...
	 * Returns: Iterator to provide BedFeature(s) for all chromosome regions.
	 * Error conditions: 1) An empty iterator is returned if region has no data
	 * available 2) A null object is returned if the file is not BigWig.(see
	 * isBigWigFile method)
	 */
	...
	/**
	 * Returns an iterator for zoom level records for the chromosome selection
	 * region.
	 * <p/>
	 * Note: the BBFile can be BigBed or BigWig.
	 * <p/>
	 * Parameters: zoomLevel - zoom level for data extraction; levels start at 1
	 * startChromosome - start chromosome name startBase - staring base position
	 * for features endChromosome - end chromosome name endBase - ending base
	 * position for feature contained - flag specifies bed features must be
	 * contained in the specified base region if true; else can intersect the
	 * region if false
	 * <p/>
	 * Returns: Iterator to provide BedFeature(s) for the requested chromosome
	 * region. Error conditions: 1) An empty iterator is returned if region has
	 ...
	 * <p/>
	 * Parameters: zoomLevel - zoom level for data extraction; levels start at 1
	 * selectionRegion - chromosome selection region consists of: startChromID -
	 * ID of starting chromosome startBase - staring base position for features
	 * endChromID - ID of endind chromosome endBase - ending base position for
	 * feature contained - flag specifies bed features must be contained in the
	...
	/*
	 * Method generates a chromosome bounds region for the supplied chromosome
	 * region name.
	 * 
	 * Note: No attempt is made to verify the region exists in the file data,
	 * nor which data is being examined.
	 * 
	 * Parameters: startChromosome - name of start chromosome startBase -
	 * starting base position for region endChromosome - name of end chromosome
	 * endBase - ending base position for region
	 * 
	 * Returns: Chromosome bounds of a named chromosome region for data
	 * extraction; or null for regions not found in the B+ chromosome index
	 * tree.
	 */
	 ...
	 * 
	 * Returns: Data count of the number of data records: number of Bed features
	 * for BigBed number of Wig sections for BigWig
	 */
